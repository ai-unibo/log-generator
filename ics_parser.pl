:- module(ics_parser,
	 [translate_ics/2,
      ...
      download_ics/1,
	  translate_ics_files_mem_to_mem/2]).
...
translate_ics_files_mem_to_mem(HandleInput,HandleOutput) :-
	open_memory_file(HandleOutput,write,Stream),
	write(Stream,':-module(ics,[ics/2]).'),nl(Stream),nl(Stream),
	%%  parse_ics(InFile,ICS), %%% SUBSTITUTED WITH:
		%%  memory_file_to_string(HandleInput,FileString),
		memory_file_to_codes(HandleInput,FileString),
		phrase(elementList(FileString2),FileString),
		drop_whites(FileString2, NoWhitesString),
		phrase(ic_list(ICS,1), NoWhitesString),
